...
import org.egov.edcr.entity.DcrReportFloorDetail;
...
import org.egov.edcr.entity.utility.DcrReportPlanDetail;
import org.egov.edcr.entity.utility.VirtualBuildingReport;
...
public class PlanReportService {
    ...
    public InputStream generateDynamicReport(PlanDetail planDetail, DXFDocument doc, EdcrApplication dcrApplication) {
        // reportService = new ReportService();
        FastReportBuilder drb = new FastReportBuilder();
        List<Map> inputData = new ArrayList<>();
        try {
            List<ScrutinyDetail> scrutinyDetails = planDetail.getReportOutput().getScrutinyDetails();
            ...
            drb.addColumn("RuleNo", "RuleNo", String.class.getName(), 100)
                    .addColumn("description", "description", String.class.getName(), 100);
            drb.setTitle("FAR");
    ...
    private Subreport getBlkDetails(DcrReportBlockDetail dcrReportBlockDetail, boolean isProposed) {
        try {
            ...
            AbstractColumn floor = ColumnBuilder.getNew()
                    .setColumnProperty("floorNo", String.class.getName())
                    .setTitle("Floor").setWidth(50)
            ...
            AbstractColumn builtUpArea = ColumnBuilder.getNew()
                    .setColumnProperty("builtUpArea", BigDecimal.class.getName())
                    .setTitle("Built Up Area in m²").setWidth(120)
            ...
            AbstractColumn carpetArea = ColumnBuilder.getNew()
                    .setColumnProperty("carpetArea", BigDecimal.class.getName())
                    .setTitle("Carpet Area in m²").setWidth(120)
                    .setStyle(reportService.getNumberStyle())
                    .build();
            ...
            if (dcrReportBlockDetail.getBlockNo() != null)
                if (isProposed) {
                    ...
                    String coveredAreaText = "1. Covered Area is "
                            + (dcrReportBlockDetail.getCoverageArea() != null
                                    ? dcrReportBlockDetail.getCoverageArea().setScale(DcrConstants.DECIMALDIGITS_MEASUREMENTS,
                                            DcrConstants.ROUNDMODE_MEASUREMENTS)
                                    : BigDecimal.ZERO)
                            + " m²";

                    String blgHgtText = "2. Height of building is "
                            + (dcrReportBlockDetail.getBuildingHeight() != null
                    ...
                    AutoText autoText = new AutoText(text.toString(),
    ...
    private Subreport getAreaDetails(boolean isProposed) {
        try {
            ...
            AbstractColumn builtUpArea = ColumnBuilder.getNew()
                    .setColumnProperty(isProposed ? "proposedBuitUpArea" : "totalExistingBuiltUpArea", BigDecimal.class.getName())
                    .setTitle("Built Up Area in m²").setWidth(120)
            ...
            AbstractColumn carpetArea = ColumnBuilder.getNew()
                    .setColumnProperty(isProposed ? "proposedCarpetArea" : "totalExistingCarpetArea", BigDecimal.class.getName())
                    .setTitle("Carpet Area in m²").setWidth(120)
                    .setStyle(reportService.getTotalNumberStyle())
                    .build();
            ...
            sub.setStyle(style);
            if (isProposed)
                sub.setDatasource(new DJDataSource("Total Proposed Details", DJConstants.DATA_SOURCE_ORIGIN_PARAMETER, 0));
            else
                sub.setDatasource(new DJDataSource("Total Existing Details", DJConstants.DATA_SOURCE_ORIGIN_PARAMETER, 0));
            sub.setLayoutManager(new ClassicLayoutManager());
            return sub;
        } catch (ColumnBuilderException e) {
            e.printStackTrace();
    ...
    private Subreport getTotalAreaDetails() {
        try {
            ...
            AbstractColumn builtUpArea = ColumnBuilder.getNew()
                    .setColumnProperty("totalBuitUpArea", BigDecimal.class.getName())
                    .setTitle("Built Up Area in m²").setWidth(120)
                    .setStyle(reportService.getTotalNumberStyle())
                    .build();
            ...
            AbstractColumn carpetArea = ColumnBuilder.getNew()
                    .setColumnProperty("totalCarpetArea", BigDecimal.class.getName())
                    .setTitle("Carpet Area in m²").setWidth(120)
                    .setStyle(reportService.getTotalNumberStyle())
                    .build();

            AbstractColumn coverageArea = ColumnBuilder.getNew()
                    .setColumnProperty("totalCoverageArea", BigDecimal.class.getName())
                    .setTitle("Covered Area in m²").setWidth(120)
                    .setStyle(reportService.getTotalNumberStyle())
                    .build();
    ...
    public InputStream generateReport(PlanDetail planDetail, DXFDocument doc, EdcrApplication dcrApplication) {
        ...
        if (dcrApplication.getPlanInformation() != null
            ...
            planDetail.getPlanInformation().setApplicantName(dcrApplication.getPlanInformation().getApplicantName());

        if (!planDetail.getDeclaredOccupancies().isEmpty()) {
            List<String> occupancies = new ArrayList<>();
            planDetail.getDeclaredOccupancies().forEach(occ -> {
                occupancies.add(occ.getType().getOccupancyTypeVal());
            });
            planDetail.getPlanInformation().setOccupancy(occupancies.stream().map(String::new).collect(Collectors.joining(",")));
        }
        ...
        final JRDataSource ds = new JRBeanCollectionDataSource(Collections.singletonList(planDetail));
        new JRBeanCollectionDataSource(planDetail.getReportOutput().getRuleOutPuts());
        String voltages = "";
        StringBuilder voltageString = new StringBuilder();

        if (!planDetail.getElectricLine().isEmpty()) {
            for (ElectricLine electricLine : planDetail.getElectricLine())
        ...
        valuesMap.put("applicationDate", DateUtils.toDefaultDateFormat(dcrApplication.getApplicationDate()));
        ...
        valuesMap.put("far", planDetail.getFar());
        valuesMap.put("coverage", planDetail.getCoverage());
        valuesMap.put("totalFloorArea", planDetail.getVirtualBuilding() != null
                ? planDetail.getVirtualBuilding().getTotalFloorArea()
                : BigDecimal.valueOf(0));
        valuesMap.put("totalBuiltUpArea", planDetail.getVirtualBuilding() != null
                ? planDetail.getVirtualBuilding().getTotalBuitUpArea()
                : BigDecimal.valueOf(0));
        valuesMap.put("electricLineVoltage", voltages);
        valuesMap.put("blockCount", planDetail.getBlocks() != null && !planDetail.getBlocks().isEmpty()
                ? planDetail.getBlocks().size()
                : 0);
        ...
        if (clientSpecificSubReport) {
            ...
            List<DcrReportBlockDetail> existingBlockDetails = buildBlockWiseExistingInfo(planDetail);
            VirtualBuildingReport virtualBuildingReport = buildVirtualBuilding(planDetail.getVirtualBuilding());
            ...
            List<DcrReportBlockDetail> proposedBlockDetails = buildBlockWiseProposedInfo(planDetail);
            ...
            for (String blkName : blocks.keySet()) {
                List blkHeading = new ArrayList();
                blkHeading.add(BLOCK + blkName);
                drb.addConcatenatedReport(createHeaderSubreport("Block " + blkName + " - Scrutiny Details", BLOCK + blkName));
        ...
        planDetail.setEdcrPassed(finalReportStatus);
    ...
    private List<DcrReportBlockDetail> buildBlockWiseProposedInfo(PlanDetail planDetail) {
                        ...
                        for (Floor floor : floors) {
                            ...
                            if (occupancies.size() > 0)
                                for (Occupancy occupancy : occupancies) {
                                    ...
                                    dcrReportFloorDetail
                                            .setFloorArea(occupancy.getExistingFloorArea().compareTo(BigDecimal.ZERO) > 0
                                                    ? occupancy.getFloorArea().subtract(occupancy.getExistingFloorArea())
                                                    : occupancy.getFloorArea());
                                    dcrReportFloorDetail
                                            .setCarpetArea(occupancy.getExistingCarpetArea().compareTo(BigDecimal.ZERO) > 0
                            ...
                            if (mezzanineFloors != null && mezzanineFloors.size() > 0)
                                for (MezzanineFloor mezzanineFloor : mezzanineFloors) {
                                    ...
                                    dcrReportFloorDetail.setOccupancy(mezzanineFloor.getOccupancyType() != null
                                            ? mezzanineFloor.getOccupancyType().getOccupancyTypeVal()
                                            : "");
    ...
    private List<DcrReportBlockDetail> buildBlockWiseExistingInfo(PlanDetail planDetail) {
        List<DcrReportBlockDetail> dcrReportBlockDetails = new ArrayList<>();
