...
public class MinDistance {
    ...
    public static BigDecimal getYardMinDistance(PlanDetail pl, String name, String level, DXFDocument doc) {
        new Util();
        ...
        DXFLWPolyline buildFoorPrint = setBackByLevel.getBuildingFootPrint().getPolyLine();
        Yard yard = null;
        if (name.contains(DxfFileConstants.FRONT_YARD) || name.contains(DxfFileConstants.BSMNT_FRONT_YARD)) {
            yard = setBackByLevel.getFrontYard();
            yardPolyline = setBackByLevel.getFrontYard().getPolyLine();
        } else if (name.contains(DxfFileConstants.REAR_YARD) || name.contains(DxfFileConstants.BSMNT_REAR_YARD)) {
            yard = setBackByLevel.getRearYard();
            yardPolyline = setBackByLevel.getRearYard().getPolyLine();
        } else if (name.contains(DxfFileConstants.SIDE_YARD_1) || name.contains(DxfFileConstants.BSMNT_SIDE_YARD_1)) {
            yard = setBackByLevel.getSideYard1();
            yardPolyline = setBackByLevel.getSideYard1().getPolyLine();
        } else if (name.contains(DxfFileConstants.SIDE_YARD_2) || name.contains(DxfFileConstants.BSMNT_SIDE_YARD_2)) {
            yardPolyline = setBackByLevel.getSideYard2().getPolyLine();
        ...
        if (level.equals(-1) && (plotBoundary == null || buildFoorPrint == null || yard == null)) {
            pl.getErrors().put("Set back calculation Error", "Either" + DxfFileConstants.BSMNT_FOOT_PRINT + ","
                    + DxfFileConstants.PLOT_BOUNDARY + " or " + name + " is not found");
            ...
            pl.getErrors().put("Set back calculation Error", "Either " + DxfFileConstants.BUILDING_FOOT_PRINT + ","
                    + DxfFileConstants.PLOT_BOUNDARY + " or " + name + " is not found at level " + split[3]);
            return BigDecimal.ZERO.setScale(DcrConstants.DECIMALDIGITS_MEASUREMENTS);
        }
        ...
        if (!plotBoundary.isClosed())
            pl.getErrors().put("Plot boundary not closed", DxfFileConstants.PLOT_BOUNDARY + " is not closed ");

        if (level.equals(-1) && !buildFoorPrint.isClosed())
            pl.getErrors().put("Building basement foot print not closed",
                    DxfFileConstants.BSMNT_FOOT_PRINT + " is not closed for ");
        else if (!buildFoorPrint.isClosed())
            pl.getErrors().put("Building foot print not closed",
                    buildFoorPrint.getLayerName() + " is not closed ");
        ...
        List<BigDecimal> yardWidthDistance = Util.getListOfDimensionByColourCode(doc, name,
                DxfFileConstants.YARD_DIMENSION_COLOR);
        // if yard dimension defined in plan, then use the same other wise calculate manually.
        ...
        // If minimum yard distance defined, then use the same. Here we are overwrite the value which is calculated in
        ...
        if (yardInSidePoints.isEmpty() || yardInSidePoints.size() == 1)
            if (level.equals(-1))
                pl.getErrors().put("Set back calculation error for basementfootprint " + name,
                        "Points of " + name + " not properly on " + DxfFileConstants.BSMNT_FOOT_PRINT);
            else
                pl.getErrors().put("Set back calculation error for footprint" + name,
        ...
        if (outsidePoints.isEmpty() || outsidePoints.size() == 1)
            pl.getErrors().put("Set back calculation error for boundary" + name,
                    "Points of " + name + " not properly on " + DxfFileConstants.PLOT_BOUNDARY);
        ...
        if (yard.getMean().doubleValue() == 0.0d) {
            ...
            if (!avgList.isEmpty()) {
                ...
                yard.setMean(BigDecimal.valueOf(avg).setScale(DcrConstants.DECIMALDIGITS_MEASUREMENTS, RoundingMode.HALF_UP));
    ...
    public static Double getSideForMean(List<Point> yardInSidePoints, List<Point> yardOutSidePoints, Yard yard1) {
        DXFLWPolyline yard = yard1.getPolyLine();
        Double distance = 0d;
