...
public class PrintUtil {
    ...
    public static void print(List<Floor> floors) {
        if (LOG.isDebugEnabled())
            for (Floor floor : floors) {
                LOG.debug("Floor Name" + floor.getName());
                if (floor.getExterior() != null) {
                    LOG.debug("Ext points Count" + floor.getExterior().getPolyLine().getVertexCount());
                    LOG.debug("maxx  : " + floor.getExterior().getPolyLine().getBounds().getMaximumX() + " minx :  "
                            + floor.getExterior().getPolyLine().getBounds().getMinimumX());
                    LOG.debug("maxy  : " + floor.getExterior().getPolyLine().getBounds().getMaximumY() + " minx :  "
                            + floor.getExterior().getPolyLine().getBounds().getMinimumY());
                    print(floor.getExterior().getPolyLine(), floor.getName());
                }
                LOG.debug("Habitable Rooms count" + floor.getHabitableRooms().size());
                int i = 0;
                for (Room r : floor.getHabitableRooms()) {
                    LOG.debug("maxx  : " + r.getPolyLine().getBounds().getMaximumX() + " minx :  "
                            + r.getPolyLine().getBounds().getMinimumX());
                    LOG.debug("maxy  : " + r.getPolyLine().getBounds().getMaximumY() + " minx :  "
                            + r.getPolyLine().getBounds().getMinimumY());
    ...
    public static void print(ReportOutput ro) {
        if (LOG.isDebugEnabled()) {
                            ...
                            for (SubRuleOutput so : rp.getSubRuleOutputs()) {
                                ...
                                if (ruleReportOutputs != null)
                                    for (RuleReportOutput rro : ruleReportOutputs) {
                                        LOG.debug("Actual: " + rro.actualResult);
                                        LOG.debug("Expected: " + rro.expectedResult);
                                        LOG.debug("Filed Verified: " + rro.fieldVerified);
                                        LOG.debug("Status: " + rro.status);
            ...
            LOG.debug("ReportOutput Completed");
        }
