...
public class BpaReportsController extends BpaGenericApplicationController {
    ...
    public String viewUtilizedSlotDetailsByApplicationHelper(@RequestParam final String applicationType,
            @RequestParam final String scheduleType,
    ...
    public String getPersonalRegisterResult(@ModelAttribute final SearchBpaApplicationForm searchBpaApplicationForm) {
        ...
        if (searchBpaApplicationForm.getUserId() == null)
            userIds.addAll(
                    userService.getUserByTypeInOrder(UserType.EMPLOYEE).stream().map(User::getId).collect(Collectors.toList()));
        else
            userIds.add(searchBpaApplicationForm.getUserId());
        final List<PersonalRegisterHelper> searchResultList = personalRegisterReportService
                .searchPersonalRegisterDetail(searchBpaApplicationForm, userIds, securityUtils.getCurrentUser());
        return new StringBuilder(DATA)
    ...
    private void prepareUserBndryAndFormData(Model model) {
        ...
        if (UserType.EMPLOYEE.equals(currentUser.getType()) && !isAdmin) {
            BoundaryType revenueType = boundaryTypeService.getBoundaryTypeByNameAndHierarchyTypeName(WARD,
            ...
            mappedElectionWard.stream().forEach(elecWard -> {
                revenueWards.addAll(crossHierarchyService.getParentBoundaryByChildBoundaryAndParentBoundaryType(elecWard.getId(),
                        revenueType.getId()));
            });
            ...
            model.addAttribute("zones",
                    employeeMappedZone.stream().sorted(Comparator.comparing(Boundary::getId)).collect(Collectors.toSet()));
            model.addAttribute("wards",
                    revenueWards.stream().sorted(Comparator.comparing(Boundary::getId)).collect(Collectors.toSet()));
            model.addAttribute("electionwards",
                    mappedElectionWard.stream().sorted(Comparator.comparing(Boundary::getId)).collect(Collectors.toSet()));
