...
public class StakeHolderController {
    ...
    private void prepareModel(final Model model, final StakeHolder stakeHolder) {
        ...
        model.addAttribute("isEmployee", securityUtils.getCurrentUser().getType().equals(UserType.EMPLOYEE)
    ...
    public String createOnlineStakeholder(@Valid @ModelAttribute(STAKE_HOLDER) final StakeHolder stakeHolder,
        ...
        StakeHolder existingStakeholder = stakeHolderService.validateStakeHolderIsRejected(stakeHolder.getMobileNumber(),
                stakeHolder.getEmailId(), stakeHolder.getAadhaarNumber(), stakeHolder.getPan(), stakeHolder.getLicenceNumber());
        ...
        redirectAttributes.addFlashAttribute(MESSAGE, messageSource.getMessage("msg.create.stakeholder.ctizen.request",
                new String[] { stakeHolderRes.getStakeHolderType().getStakeHolderTypeVal(), stakeHolderRes.getCode() }, null));
        return RDRCT_STKHLDR_RSLT + stakeHolderRes.getId();
    }
    ...
    public String approveStakeHolder(@Valid @ModelAttribute final StakeHolder stakeHolder, final Model model,
        ...
        if ("Approve".equals(workFlowAction)) {
            ...
            redirectAttributes.addFlashAttribute(MESSAGE,
                    messageSource.getMessage("msg.approve.stakeholder.success",
                            new String[] { stakeHolder.getStakeHolderType().getStakeHolderTypeVal() }, null));
            ...
            redirectAttributes.addFlashAttribute(MESSAGE,
                    messageSource.getMessage("msg.reject.stakeholder", new String[] { stakeHolderResponse.getComments() }, null));
        } else if ("Block".equals(workFlowAction)) {
            bpaSmsAndEmailService.sendSmsAndEmailOnBlockAndUnblock(stakeHolderResponse);
            redirectAttributes.addFlashAttribute(MESSAGE,
                    messageSource.getMessage("msg.block.stakeholder", new String[] { stakeHolderResponse.getComments() }, null));
        }
