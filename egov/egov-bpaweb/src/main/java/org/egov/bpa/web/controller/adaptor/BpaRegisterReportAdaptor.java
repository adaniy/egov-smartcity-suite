...
public class BpaRegisterReportAdaptor implements DataTableJsonAdapter<BpaRegisterReportHelper> {
    ...
    public JsonElement serialize(final DataTable<BpaRegisterReportHelper> bpaRegisterResponse, final Type type,
        ...
        bpaRegisterResult.forEach(baseForm -> {
            ...
            jsonObject.addProperty("totalFloorArea", baseForm.getTotalFloorArea().compareTo(BigDecimal.ZERO) == 0 ? N_A
                    : String.valueOf(baseForm.getTotalFloorArea()));
            jsonObject.addProperty("numberOfFloors",
                    baseForm.getNumberOfFloors() == null ? N_A : String.valueOf(baseForm.getNumberOfFloors()));
            jsonObject.addProperty("far",
                    baseForm.getFar().compareTo(BigDecimal.ZERO) == 0 ? N_A : String.valueOf(baseForm.getFar()));
            ...
            jsonObject.addProperty("finalApprovalDate", baseForm.getFinalApprovalDate() != null
                    ? toDefaultDateFormat(baseForm.getFinalApprovalDate())
                    : N_A);
            jsonObject.addProperty("rejectionReason", defaultString(baseForm.getRejectionReason(), N_A));
            jsonObject.addProperty("ocApprovalDate", baseForm.getOcApprovalDate() != null
                    ? toDefaultDateFormat(baseForm.getOcApprovalDate())
                    : N_A);
            jsonObject.addProperty("ocNumber", defaultString(baseForm.getOcNumber()));
