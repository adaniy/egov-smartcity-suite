...
import org.egov.bpa.transaction.service.oc.OccupancyCertificateFeeCalculation;
...
public class UpdateOccupancyCertificateFeeController {
    ...
    @Autowired
    protected ApplicationFeeCommonService applicationFeeCommonService;
    @Autowired
    ...
    public String calculateFeeform(final Model model, @PathVariable final String applicationNumber,
        ...
        if (ocFee != null && ocFee.getOc() != null) {
            ...
            if (feeCalculationMode.equalsIgnoreCase(BpaConstants.AUTOFEECAL) ||
                ...
                if (ocFee.getApplicationFeeCommon() == null) {
                    ocFee.setApplicationFeeCommon(new ApplicationFeeCommon());
                    ocFee.getApplicationFeeCommon().setStatus(bpaStatusService
                            .findByModuleTypeAndCode(BpaConstants.BPASTATUS_MODULETYPE_REGISTRATIONFEE,
                                    BpaConstants.APPROVED));
                    ocFee.getApplicationFeeCommon().setFeeDate(new Date());

                    for (BpaFee bpaFee : bpaSanctionFees) {
                        ApplicationFeeDetailCommon applicationDtl = new ApplicationFeeDetailCommon();
                        applicationDtl.setApplicationFeeCommon(ocFee.getApplicationFeeCommon());
                        applicationDtl.setBpaFee(bpaFee);
                        applicationDtl.setAmount(BigDecimal.ZERO);
                        ocFee.getApplicationFeeCommon().addApplicationFeeDetail(applicationDtl);
                    }
                }
                model.addAttribute(OC_FEE, ocFee);

                BigDecimal amount = ocFee.getApplicationFeeCommon().getApplicationFeeDetailCommon().stream()
                        .map(ApplicationFeeDetailCommon::getAmount)
    ...
    public String calculateFeeform(@Valid @ModelAttribute(OC_FEE) OccupancyFee ocFee,
        ...
        ApplicationFeeCommon applicationFeeCommon = applicationFeeCommonService
                .saveApplicationFee(ocFee.getApplicationFeeCommon());
        ocFee.setApplicationFeeCommon(applicationFeeCommon);
        EgDemand demand = bpaDemandService.generateDemandUsingSanctionFeeList(ocFee.getApplicationFeeCommon(),
        ...
        return org.apache.commons.lang.StringUtils.isBlank(applicationFeeCommon.getModifyFeeReason()) ? CREATEOCFEE_VIEW
                : MODIFYOCFEE_VIEW;
    }
}
