...
<div class="row">
	<div class="col-md-12">
		<form:form role="form" action="" method="post"
			...
			<input type="hidden"  id="occupancyFee" value="${occupancyFee.id}"/>
				...
				<div class="panel-body custom-form ">
					...
					<div class="form-group">
						<div class="col-sm-3 add-margin">
							...
							<form:hidden path="applicationFeeCommon.status" id="feeapplicationstatusid"
								value="${occupancyFee.applicationFeeCommon.status.id}" />
					...
					<c:choose>
						<c:when test="${!occupancyFee.applicationFeeCommon.applicationFeeDetailCommon.isEmpty()}">
							...
							<c:forEach var="docs"
								items="${occupancyFee.applicationFeeCommon.applicationFeeDetailCommon}"
								varStatus="status">
								<div class="form-group">
									<div class="col-sm-5 add-margin check-text text-right">
										<c:out value="${docs.bpaFee.description}" />
										<form:hidden id="applicationFeeDetailCommon${status.index}id"
											path="applicationFeeCommon.applicationFeeDetailCommon[${status.index}].id"
											value="${docs.id}" />
										<form:hidden id="applicationFeeDetailCommon${status.index}bpaFee"
											path="applicationFeeCommon.applicationFeeDetailCommon[${status.index}].bpaFee"
											value="${docs.bpaFee.id}" />
										<form:hidden
											id="applicationFeeDetailCommon${status.index}applicationFeeCommon"
											path="applicationFeeCommon.applicationFeeDetailCommon[${status.index}].applicationFeeCommon"
											value="${docs.applicationFeeCommon.id}" />
										...
										<c:choose>
											<c:when
												...
												<form:input class="form-control patternvalidation text-right otherFees"
													...
													id="applicationFeeDetailCommon${status.index}amount"
													value="${docs.amount}"
													path="applicationFeeCommon.applicationFeeDetailCommon[${status.index}].amount" />
											</c:when>
											<c:otherwise>
					...
					<div class="form-group">
						...
						<div class="col-sm-7 add-margin text-center">
							<form:textarea class="form-control patternvalidation"
								...
								id="modifyFeeReason" path="applicationFeeCommon.modifyFeeReason" required="required" />
							<form:errors path="applicationFeeCommon.modifyFeeReason"
								cssClass="add-margin error-msg" />
						</div>
					</div>
