				...
				<c:when test="${not empty inspectionList}">
					<c:forEach items="${inspectionList}" var="inspn" varStatus="status">
						<tr id="lprow">
							...
							<td align="center" class="view-content" style="font-size: 90%;">
								<c:out
									value="${inspn.inspection.inspectedBy.username.concat('::').concat(inspn.inspection.inspectedBy.name)}" />
							</td>
							<td align="center" style="font-size: 90%;"><a
								class="view-content" style="cursor: pointer; font-size: 15px;"
								onclick="window.open('/bpa/application/occupancy-certificate/show-inspection-details/${applicationNumber}/${inspn.inspection.inspectionNumber}','view','width=600, height=400,scrollbars=yes')">
									<i class="fa fa-eye" aria-hidden="true"> <spring:message
											code="lbl.view" /></i>
							</a> &nbsp;&nbsp;&nbsp; <c:if
									test="${showEditInspection && showEditInspectionIndex eq status.index}">
									<a class="view-content"
										style="cursor: pointer; font-size: 15px;"
										onclick="window.open('/bpa/application/occupancy-certificate/update-inspection/${applicationNumber}/${inspn.inspection.inspectionNumber}','view','width=600, height=400,scrollbars=yes')">
										<i class="fa fa-edit" aria-hidden="true"> <spring:message
												code="lbl.edit" /></i>
									</a>
								</c:if></td>
						</tr>
...
<script>
	$(document).ready(function() {
		/*$("#ocInspectionDetails tbody tr:gt(1)").each(function(index) {
			$(this).find('a').hide();
		});*/
	});
</script>