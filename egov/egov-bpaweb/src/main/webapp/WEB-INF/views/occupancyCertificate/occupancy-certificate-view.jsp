...
<div class="row">
	<div class="col-md-12">
		<form:form role="form" action="../update-submit" method="post"
			...
			<input type="hidden" id="captureNocVerifyRemarks"
				value="${captureNocVerifyRemarks}">
			<ul class="nav nav-tabs" id="settingstab">
				...
				<c:if test="${captureNocVerifyRemarks}">
					<li><a data-toggle="tab" href="#noc-verify-remarks"
						   data-tabidx=6><spring:message code='lbl.noc.verify.remark' /></a></li>
				</c:if>
				<c:if test="${captureTSRemarks}">
					<li><a data-toggle="tab" href="#ts-remarks" data-tabidx=7><spring:message
								code='lbl.town.surveyor.remarks' /></a></li>
				</c:if>
				<c:if test="${not empty letterToPartyList}">
					<li><a data-toggle="tab" href="#view-lp" data-tabidx=8><spring:message
								code='lbl.lp.details' /></a></li>
				</c:if>
				<c:if test="${not empty occupancyCertificate.occupancyFee}">
					<li><a data-toggle="tab" href="#view-fee" data-tabidx=9><spring:message
								code='lbl.ocfee.details' /></a></li>
				</c:if>
				<c:if test="${not empty occupancyCertificate.getOcNocVerifications()}">
					<li><a data-toggle="tab" href="#view-noc-verify-dtls"
						   data-tabidx=10><spring:message code='lbl.noc.verify.dtl' /></a></li>
				</c:if>
			</ul>
			<div class="tab-content">
				...
				<c:if test="${captureNocVerifyRemarks}">
					<div id="noc-verify-remarks" class="tab-pane fade">
						<div class="panel panel-primary" data-collapsed="0">
							<jsp:include page="oc-noc-verification.jsp"></jsp:include>
						</div>
					</div>
				</c:if>
				<c:if test="${not empty occupancyCertificate.getOcNocVerifications()}">
					<div id="view-noc-verify-dtls" class="tab-pane fade">
						<div class="panel panel-primary" data-collapsed="0">
						<c:set var="nocVerifications" value="${occupancyCertificate.getOcNocVerifications()}" scope="request"></c:set>
							<jsp:include page="../application/view-noc-verification.jsp"></jsp:include>
						</div>
					</div>
				</c:if>
				<c:if test="${not empty letterToPartyList}">
	...
	<div class="modal-dialog" role="document">
		...
		<div class="modal-content">
			<div class="modal-header text-center">
				<h4 class="modal-title w-100 font-weight-bold alert alert-info">Reason
					For Revert</h4>
			</div>
