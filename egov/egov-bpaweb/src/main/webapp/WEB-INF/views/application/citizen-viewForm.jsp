	...
	<div class="col-md-12">
		<form:form role="form"
			...
			<ul class="nav nav-tabs" id="settingstab">
				...
				<c:if test="${not empty bpaApplication.getPermitNocVerifications()}">
					<li><a data-toggle="tab" href="#view-noc-verify-dtls"
						   data-tabidx=8><spring:message code='lbl.noc.verify.dtl' /></a></li>
				</c:if>
			</ul>
			<div class="tab-content">
				...
				<div id="appliccation-info" class="tab-pane fade in active">
					<c:if test="${bpaApplication.permitCancel ne null}">
						<div class="panel panel-primary" data-collapsed="0">
							<jsp:include page="view-permit-cancel-details.jsp"></jsp:include>
						</div>
					</c:if>
					<div class="panel panel-primary" data-collapsed="0">
					...
					<c:if test="${bpaApplication.siteDetail[0].demolitionArea > 0}">
				...
				<c:if test="${not empty bpaApplication.getPermitNocVerifications()}">
					<div id="view-noc-verify-dtls" class="tab-pane fade">
						<div class="panel panel-primary" data-collapsed="0">
						<c:set var="nocVerifications" value="${bpaApplication.getPermitNocVerifications()}" scope="request"></c:set>
							<jsp:include page="view-noc-verification.jsp"></jsp:include>
						</div>
					</div>
				</c:if>
			</div>
					...
					<tr>
						<td><c:if
								...
								<form:button type="button" id="buttonCancel"
									class="btn btn-danger" value="Cancel Application"> Cancel Application </form:button>
							</c:if></td>
						...
						<c:if
							test="${onlinePaymentEnable && isFeeCollected && (bpaApplication.status.code eq 'Registered' || bpaApplication.status.code eq 'Scheduled For Document Scrutiny' || bpaApplication.status.code eq 'Approved') }">
							<td><a
						...
						<c:if
							...
							<td><a
								...
								onclick="window.open('/bpa/application/generatepermitorder/${bpaApplication.applicationNumber}','popup','width=1100,height=700'); return false;">
									Print Permit Order </a></td>
						</c:if>
						<c:if
							test="${bpaApplication.status.code eq 'Revocation approved' }">
							<td><a
								href="/bpa/application/revocation/generateorder/${bpaApplication.revocations[0].revocationNumber}"
								target="popup" class="btn btn-primary"
								onclick="window.open('/bpa/application/revocation/generateorder/${bpaApplication.revocations[0].revocationNumber}','popup','width=1100,height=700'); return false;">
									Print Permit Revocation Order </a></td>
						</c:if>

						<c:if
							test="${(bpaApplication.status.code eq 'Rejected' || bpaApplication.status.code eq 'Cancelled') && bpaApplication.state ne null}">
							<td><a
