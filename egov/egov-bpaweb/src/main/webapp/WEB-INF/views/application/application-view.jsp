...
<div class="row">
	<div class="col-md-12">
		<form:form role="form" action="/bpa/application/update-submit/${bpaApplication.applicationNumber}" method="post" modelAttribute="bpaApplication"
			...
			<input type="hidden" id="captureNocVerifyRemarks" value="${captureNocVerifyRemarks}">
			<ul class="nav nav-tabs" id="settingstab">
				...
				<c:if test="${captureNocVerifyRemarks}">
					<li><a data-toggle="tab" href="#noc-verify-remarks"
						   data-tabidx=5><spring:message code='lbl.noc.verify.remark' /></a></li>
				</c:if>
				<c:if test="${showUpdateNoc}">
					<li><a data-toggle="tab" href="#checklist-info" data-tabidx=6><spring:message
							code='lbl.noc.doc.details' /></a></li>
				</c:if>
				<c:if test="${!showUpdateNoc && not empty bpaApplication.applicationNOCDocument}">
					<li><a data-toggle="tab" href="#noc-info" data-tabidx=6><spring:message
							code='lbl.noc.details' /></a></li>
				</c:if>
				<c:if test="${not empty bpaApplication.applicationFee}">
					<li><a data-toggle="tab" href="#view-fee" data-tabidx=7><spring:message
				...
				<c:if test="${not empty bpaApplication.getPermitNocVerifications()}">
					<li><a data-toggle="tab" href="#view-noc-verify-dtls"
						   data-tabidx=9><spring:message code='lbl.noc.verify.dtl' /></a></li>
				</c:if>
			</ul>
			<div class="tab-content">
				...
				<c:if test="${captureNocVerifyRemarks}">
					<div id="noc-verify-remarks" class="tab-pane fade">
						<div class="panel panel-primary" data-collapsed="0">
							<jsp:include page="permit-noc-verification.jsp"></jsp:include>
						</div>
					</div>
				</c:if>
				<c:if test="${not empty bpaApplication.getPermitNocVerifications()}">
