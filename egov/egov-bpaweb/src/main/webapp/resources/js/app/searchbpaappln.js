...
$(document)
		.ready(
				function() {
					...
					var revokeorderurl = '/bpa/application/revocation/generateorder/';
					$('#btnSearch').click(function() {
						var isValid = false;
						$('#searchBpaApplicationForm')
						.find(':input',':select',':textarea')
						.not(':input[type=button], :input[type=submit], :input[type=hidden]').each(function() {
							var input = $(this);
							if(input.attr('type') == 'checkbox') {
								if($("#isReqularized:checked").val()) {
									isValid =true;
	                                return false;
								} else {
									isValid =false;
											...
											ajax : {
                                                ...
                                                data: function (args) {
                                                    return {
                                                        ...
                                                        "reqularized": $("#isReqularized:checked").val(),
											...
											columns : [
													...
													},
													{
														...
														"render" : function(
															...
															} else if (row.status == 'Order Issued to Applicant' || row.status == 'Revocation cancelled') {
																return ('<select class="dropchange" style="width:160px;font-size: small">'+commonOptions+'<option  value='
																		+ permitorderurl
																		+ row.applicationNumber + '>Generate Permit Order</option></select>');
															} else if (row.status == 'Revocation approved') {
																return ('<select class="dropchange" style="width:160px;font-size: small">'+commonOptions+'<option  value='
																		+ revokeorderurl
																		+ row.revocationNumber + '>Generate Revocation Order</option></select>');
															} else if ((row.status == 'Rejected' || row.status == 'Cancelled') && row.pendingAction == 'END') {
																return ('<select class="dropchange" style="width:160px;font-size: small">'+commonOptions+'<option  value='
																		+ rejectionnoticeurl
