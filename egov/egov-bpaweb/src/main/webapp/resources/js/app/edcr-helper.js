    ...
    function ($) {
        ...
        function setBuildingFloorDetails(block, blkIdx, tableId) {
            ...
            for(var i = 0; i < block.building.floors.length; i++) {
                ...
                if(floorObj.mezzanineFloor.length > 0) {
                    for(var k = 0; k < floorObj.mezzanineFloor.length; k++) {
	                    var mezzanineFloor = floorObj.mezzanineFloor[k];
		                    if(mezzanineFloor.builtUpArea && mezzanineFloor.builtUpArea > 0) {
			                    addFloorDetailsIntoTable(blkIdx, floorIdx, $(tableId+" tbody tr").length, "Mezzanine Floor", floorObj.number, occupancyResponseByDesc[mezzanineFloor.occupancyType], mezzanineFloor.builtUpArea, mezzanineFloor.floorArea, mezzanineFloor.carpetArea);
			                    floorIdx++;
		                    }
	                    }
                    }
                        ...
                        if (response.errorDetail && response.errorDetail.errorCode != null && response.errorDetail.errorCode != '') {
                            ...
                            if ($('#loadingFloorDetailsFromEdcrRequire').val() === 'true' && $('#mode').val() && $('#mode').val() === 'new' && response.planDetail) {
                                ...
                                if (response.planDetail.blocks.length <= 0) {
                                    ...
                                    if (response.planDetail.declaredOccupancies.length > 1) {
                                        $('[name=occupancy] option').filter(function () {
                                            return ($(this).text() == 'Mixed');
                                        }).prop('selected', true);
                                    } else {
                                        $('[name=occupancy] option').filter(function () {
                                            return ($(this).text() == response.planDetail.declaredOccupancies[0].type);
                                        }).prop('selected', true);
