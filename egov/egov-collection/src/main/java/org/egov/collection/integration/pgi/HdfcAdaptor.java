...
/**
 * The PaymentRequestAdaptor class frames the payment request object and parses
 * the payment response for the payment gateway service HDFC
 */
...
public class HdfcAdaptor implements PaymentGatewayAdaptor {
    ...
    public PaymentRequest createPaymentRequest(ServiceDetails paymentServiceDetails, ReceiptHeader receiptHeader) {
        ...
        paymentRequest.setParameter(CollectionConstants.HDFC_PRODUCT_INFO, (receiptHeader.getConsumerCode().replace("-", "").replace("/", "")));

        paymentRequest.setParameter(CollectionConstants.HDFC_FIRSTNAME, buildPayeeName(receiptHeader.getPayeeName()));
        paymentRequest.setParameter(CollectionConstants.HDFC_EMAIL, !StringUtils.isBlank(receiptHeader.getPayeeEmail()) ? receiptHeader.getPayeeEmail() : collectionApplicationProperties.hdfcTransactionEmail());
        ...
        paymentRequest.setParameter(CollectionConstants.HDFC_CURL, returnUrl.toString());// If transaction is  cancelled by  customer on  PAYU page.

        //Product Info
        paymentRequest.setParameter(UDF1, (receiptHeader.getConsumerCode().replace("-", "").replace("/", "")));
        //Email ID
        paymentRequest.setParameter(UDF2, !StringUtils.isBlank(receiptHeader.getPayeeEmail()) ? receiptHeader.getPayeeEmail() : collectionApplicationProperties.hdfcTransactionEmail());
        //Phone
    ...
    public PaymentResponse parsePaymentResponse(String response) {
        ...
        try {
            ...
            if (verifyResponse(responseMap, transactionDate, onlinePaymentReceiptHeader)) {
                hdfcResponse.setAuthStatus(responseMap.get(STATUS).equalsIgnoreCase(SUCCESS) ? CollectionConstants.PGI_AUTHORISATION_CODE_SUCCESS : responseMap.get(STATUS));
    ...
    private boolean verifyResponse(Map<String, String> responseMap, Date transactionDate, ReceiptHeader onlinePaymentReceiptHeader) throws ApplicationException {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(txnId) && StringUtils.isNotBlank(status)
                ...
                /* Transaction is verified using txnid , udf1 from the response and fetching the receipt header object
                and using receipt header object data to build reverse hash data and verify the hash in response and reverse hash are same.*/
                ...
                if (onlinePaymentReceiptHeader != null) {
                    ...
                    LOGGER.info("Response Parameters : " + STATUS + " = " + status + " | " + ADDITIONAL_CHARGES + " = " + additionalCharges
                            + " | " + UDF5 + " = " + udf5 + " | " + UDF4 + " = " + udf4 +  " | udf4 from response = " + responseMap.get("udf4")+ " | " + UDF3 + " = " + udf3 + " | " + UDF2 + " = " + udf2
                            + " | " + UDF1 + " = " + udf1 + " | " + EMAIL + " = " + udf2 + " | " + FIRST_NAME + " = " + firstName + " | " + PRODUCT_INFO + " = " + productInfo
                            + " | " + AMOUNT + " = " + amount + " | " + TNX_ID + " = " + txnId + " | " + HASH + " = " + responseHash);
                    ...
                    boolean value = transactionDate.compareTo(formatDate(onlinePaymentReceiptHeader.getReceiptDate().toString())) >= 0
    ...
    public PaymentResponse createOfflinePaymentRequest(OnlinePayment onlinePayment) {
        ...
        try {
            ...
            hdfcResponse.setAuthStatus(responseMap.get(STATUS).equals(SUCCESS) ? CollectionConstants.PGI_AUTHORISATION_CODE_SUCCESS : responseMap.get(STATUS));
