...
public class ScheduleAppointmentForDocumentScrutinyService {
    ...
    public void scheduleOneDayPermit() {
        ...
        for (SlotMapping sltMapping : slotMappings) {
            List<Slot> slotList = slotRepository.findByZoneWardAndApplicationDateForOneDayPermit(sltMapping.getZone(),
                    sltMapping.getElectionWard(), calendar.getTime());
            if (!slotList.isEmpty())
                for (Slot slot : slotList) {
                    ...
                    if (!slot.getSlotDetail().isEmpty()) {
                        ...
                        List<BpaApplication> bpaApplications = getBpaApplicationsByStatusSlotsAndBoundary(sltMapping.getZone(),
                                sltMapping.getElectionWard(),
                                totalAvailableSlots);
                        if (!bpaApplications.isEmpty()) {
                            Map<Long, SlotApplication> processedApplication = new HashMap<>();
                            Map<Long, String> errorMsg = new ConcurrentHashMap<>();
    ...
    private List<BpaApplication> getBpaApplicationsByStatusSlotsAndBoundary(Boundary bndry, Boundary ward,
