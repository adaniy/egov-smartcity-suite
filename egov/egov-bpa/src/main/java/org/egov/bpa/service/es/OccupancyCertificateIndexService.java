...
public class OccupancyCertificateIndexService {
    ...
    public OccupancyCertificateIndex createOccupancyIndex(OccupancyCertificate occupancyCertificate) {
        ...
        occupancyIndex.setOccupancyCertificateNumber(occupancyCertificate.getOccupancyCertificateNumber() == null ? EMPTY
                : occupancyCertificate.getOccupancyCertificateNumber());
        occupancyIndex.setApplicationType(
                occupancyCertificate.getApplicationType() == null ? EMPTY : occupancyCertificate.getApplicationType());
        occupancyIndex.setStatus(
                occupancyCertificate.getStatus().getCode() == null ? EMPTY : occupancyCertificate.getStatus().getCode());
        occupancyIndex.setSource(
                occupancyCertificate.getSource() == null ? Source.SYSTEM.toString() : occupancyCertificate.getSource().name());
        occupancyIndex.setParentPermissionNumber(occupancyCertificate.getParent().getPlanPermissionNumber() == null ? EMPTY
                : occupancyCertificate.getParent().getPlanPermissionNumber());
        occupancyIndex.setTotalDemandAmount(occupancyCertificate.getDemand().getBaseDemand() == null ? BigDecimal.ZERO
                : occupancyCertificate.getDemand().getBaseDemand());
        occupancyIndex.setTotalCollectedAmount(occupancyCertificate.getDemand().getAmtCollected() == null ? BigDecimal.ZERO
                : occupancyCertificate.getDemand().getAmtCollected());
        ...
        if (applicationIndex != null && occupancyCertificate.getId() != null)
            ...
            applicationIndex = ApplicationIndex.builder().withModuleName(BpaConstants.APPL_INDEX_MODULE_NAME)
                    ...
                    .withAadharNumber(StringUtils.isBlank(occupancyCertificate.getParent().getOwner().getAadhaarNumber())
                            ? EMPTY
                            : occupancyCertificate.getParent().getOwner().getAadhaarNumber())
    ...
    private void buildApplicationIndexForUpdate(OccupancyCertificate occupancyCertificate, User user,
        ...
        if (occupancyCertificate.getStatus().getCode().equals(BpaConstants.APPLICATION_STATUS_CANCELLED)
                || occupancyCertificate.getStatus().getCode().equals(BpaConstants.APPLICATION_STATUS_REJECTED)) {
