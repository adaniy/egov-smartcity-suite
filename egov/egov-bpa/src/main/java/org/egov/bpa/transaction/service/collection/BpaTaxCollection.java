...
public class BpaTaxCollection extends TaxCollection {
    ...
    public void updateDemandDetails(final BillReceiptInfo billRcptInfo) {
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info(" *************Bill reference number**************" + billRcptInfo.getBillReferenceNum());
    ...
    /**
     * @param demand Updates bpaApplication Object once Collection Is done. send Record move to Commissioner and Send SMS and
     * Email after Collection
     * @throws IOException
     */
    @Transactional
    public void updateBpaApplication(final EgDemand demand) {
        ...
        if (application.getStatus().getCode().equals(BpaConstants.APPLICATION_STATUS_CREATED)) {
            ...
            final WorkFlowMatrix wfMatrix = bpaUtils.getWfMatrixByCurrentState(application.getIsOneDayPermitApplication(),
                ...
                approvalPosition = bpaUtils.getUserPositionIdByZone(wfMatrix.getNextDesignation(),
                        application.getSiteDetail().get(0) != null
                                && application.getSiteDetail().get(0).getElectionBoundary() != null
                                        ...
                                        : null);
            bpaUtils.redirectToBpaWorkFlow(approvalPosition, application, BpaConstants.WF_NEW_STATE, BpaConstants.BPAFEECOLLECT,
                    null, null);
        }
    ...
    public void updateDemandDetailForReceiptCreate(final Set<ReceiptAccountInfo> accountDetails, final EgDemand demand,
        ...
        if (LOGGER.isInfoEnabled())
            LOGGER.info(" *************Before getting application by demand, Demand Object**************" + demand);
        final BpaApplication application = applicationBpaService
                .getApplicationByDemand(demand);
        if (LOGGER.isInfoEnabled())
            LOGGER.info(" *************After Getting application by demand, Application object**************" + application);
        ...
        if (LOGGER.isInfoEnabled())
            LOGGER.info(" *************After Receipt persist**************" + application);
        if (BpaConstants.APPLICATION_STATUS_APPROVED.equalsIgnoreCase(application.getStatus().getCode())) {
            if (LOGGER.isInfoEnabled())
                LOGGER.info(" On Approve *************workflow current state**************" + application.getCurrentState());
    ...
    @Override
    public void validatePayment(String billReferenceNumber, BigDecimal actualAmountPaid,
            ArrayList<ReceiptDetail> receiptDetailsArray) {

        EgBill bill = egBillDAO.findById(Long.valueOf(billReferenceNumber), false);

        if (bill == null)
            throw new ApplicationRuntimeException("Bill not found with number " + billReferenceNumber);
        else if (!bill.getPartPaymentAllowed() &&
                bill.getTotalAmount().compareTo(actualAmountPaid) != 0)
            throw new ApplicationRuntimeException("Application fee to be paid and the collected amount is mismatching");
    }
