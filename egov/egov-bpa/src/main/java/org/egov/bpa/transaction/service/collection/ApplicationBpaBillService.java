...
public class ApplicationBpaBillService extends BillServiceInterface {
    ...
    public EgDemand createDemand(final BpaApplication application) {
        ...
        BigDecimal applicationFeeAmount = applicationBpaService.setAdmissionFeeAmountForRegistrationWithAmenities(
                application.getServiceType().getId(), application.getApplicationAmenity());
        if (application.getDemand() == null || application.getDemand() != null
                && applicationFeeAmount.compareTo(BigDecimal.ZERO) > 0
                && applicationFeeAmount.compareTo(application.getDemand().getBaseDemand()) > 0
                || applicationFeeAmount.compareTo(application.getDemand().getBaseDemand()) < 0) {
            final Map<String, BigDecimal> feeDetails = new HashMap<>();
            EgDemand egDemand = application.getDemand();
            ...
            if (installment != null && !feeDetails.isEmpty()) {
                final Set<EgDemandDetails> dmdDetailSet = new HashSet<>();
                for (final Entry<String, BigDecimal> demandReason : feeDetails.entrySet())
                    dmdDetailSet
                            .add(createDemandDetails(feeDetails.get(demandReason.getKey()), demandReason.getKey(), installment));
                if (egDemand == null) {
                    egDemand = new EgDemand();
                    egDemand.setEgInstallmentMaster(installment);
                    egDemand.getEgDemandDetails().addAll(dmdDetailSet);
                    egDemand.setIsHistory("N");
                    egDemand.setBaseDemand(applicationFeeAmount);
                    egDemand.setCreateDate(new Date());
                    egDemand.setModifiedDate(new Date());
                } else {
                    for (EgDemandDetails dmdDtl : egDemand.getEgDemandDetails())
                        egDemandDetailsDao.delete(dmdDtl);
