...
public abstract class OccupancyCertificateWorkflowCustomImpl implements OccupancyCertificateWorkflowCustom {
    ...
    private SimpleWorkflowService<OccupancyCertificate> bpaApplicationWorkflowService;
    ...
    public void createCommonWorkflowTransition(final OccupancyCertificate oc, final WorkflowBean wfBean) {
        ...
        if (oc.getCurrentState() == null) {
            ...
            oc.setStatus(getStatusByPassingCode(BpaConstants.STATUS_INIT_REJECT));
            oc.transition().progressWithStateCopy()
                    .withSenderName(user.getUsername() + BpaConstants.COLON_CONCATE + user.getName())
                    .withComments(wfBean.getApproverComments())
            ...
            ownerUser = bpaWorkFlowService.getAssignmentsByPositionAndDate(oc.getState().getOwnerPosition().getId(), new Date())
                    .get(0).getEmployee();
            oc.setStatus(getStatusByCurrentMatrixStatus(wfMatrix));
            oc.transition().progressWithStateCopy()
                    .withSenderName(user.getUsername() + BpaConstants.COLON_CONCATE + user.getName())
            ...
            if (BpaConstants.WF_CANCELAPPLICATION_BUTTON.equalsIgnoreCase(wfBean.getWorkFlowAction()))
                oc.setStatus(getStatusByPassingCode(BpaConstants.APPLICATION_STATUS_CANCELLED));
            else
                oc.setStatus(getStatusByPassingCode(BpaConstants.APPLICATION_STATUS_REJECTED));
            oc.transition().end()
                    .withSenderName(user.getUsername() + BpaConstants.COLON_CONCATE + user.getName())
            ...
            BpaStateInfo bpaStateInfo = bpaWorkFlowService.getBpaStateInfo(oc.getCurrentState(),
                    oc.getStateHistory(), oc.getTownSurveyorInspectionRequire(), new BpaStateInfo(),
                    wfMatrix, wfBean.getWorkFlowAction());
            oc.setStatus(getStatusByCurrentMatrixStatus(wfMatrix));
            oc.transition().progressWithStateCopy()
                    .withSenderName(user.getUsername() + BpaConstants.COLON_CONCATE + user.getName())
                    .withComments(wfBean.getApproverComments()).withStateValue(wfMatrix.getNextState())
            ...
            pos = positionMasterService
                    .getPositionById(bpaWorkFlowService.getNocInitiator(oc.getStateHistory(), oc.getCurrentState()));
            ownerUser = bpaWorkFlowService.getAssignmentsByPositionAndDate(oc.getState().getOwnerPosition().getId(), new Date())
                    .get(0).getEmployee();
            wfMatrix = workFlowMatrixService
                    .getWorkFlowObjectbyId(bpaWorkFlowService.getPreviousWfMatrixId(oc.getStateHistory()));
