...
public class BpaFeeService {
    ...
    public BpaFeeDetail findBpaFeeDetailbyFeeTypeAndAdditionalTypeAndArea(String additionalType, String feeCode, Double area) {
        final Criteria feeDtlCrit = getCurrentSession().createCriteria(BpaFeeDetail.class, "bpaFeeDtl")
                .createAlias("bpaFeeDtl.bpafee", "bpafee");
        feeDtlCrit.add(Restrictions.eq("bpafee.code", feeCode));
        feeDtlCrit.add(Restrictions.eq("bpaFeeDtl.additionalType", additionalType));
        final Criterion area1st = Restrictions.conjunction().add(Restrictions.le("fromAreasqmt", area))
                .add(Restrictions.ge("toAreasqmt", area));

        final Criterion area2nd = Restrictions.conjunction().add(Restrictions.le("fromAreasqmt", area))
                .add(Restrictions.isNull("toAreasqmt"));
        feeDtlCrit.add(Restrictions.disjunction().add(area1st).add(area2nd));

        return (BpaFeeDetail) feeDtlCrit.uniqueResult();
    }
}