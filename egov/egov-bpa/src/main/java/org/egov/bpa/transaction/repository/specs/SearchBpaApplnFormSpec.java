...
import static org.egov.bpa.utils.BpaConstants.APPLICATION_STATUS_REVOKE_APPROVED;
...
public final class SearchBpaApplnFormSpec {
        ...
        return (root, query, builder) -> {
            ...
            predicate.getExpressions()
                    .add(root.get(STATUS).get("code").in(APPLICATION_STATUS_ORDER_ISSUED, APPLICATION_STATUS_CANCELLED,
                            APPLICATION_STATUS_REVOKE_CANCELED, APPLICATION_STATUS_REVOKE_APPROVED));
            query.distinct(true);
            return predicate;
        };
    }
    ...
    public static Specification<BpaApplication> searchPermitRevocationSpecification(final SearchBpaApplicationForm requestForm,
        ...
        return (root, query, builder) -> {
            ...
            if (!positionIds.isEmpty())
                predicate.getExpressions()
                        .add(stateJoin.get("ownerPosition").get("id").in(positionIds));
            query.distinct(true);
            return predicate;
        };
    }

    public static Specification<BpaApplication> searchPermitCancelSpecification(final SearchBpaApplicationForm requestForm,
            final List<Long> positionIds) {
        return (root, query, builder) -> {
            final Predicate predicate = builder.conjunction();
            Join<BpaApplication, State<Position>> stateJoin = root.join("state");
            ...
            predicate.getExpressions()
                    .add(root.get(STATUS).get("code").in(APPLICATION_STATUS_PERMIT_CANCEL_INIT));
            if (!positionIds.isEmpty())
                predicate.getExpressions()
                        .add(stateJoin.get("ownerPosition").get("id").in(positionIds));
            query.distinct(true);
    ...
    private static void siteDetailSpec(SearchBpaApplicationForm requestForm, Root<BpaApplication> root, CriteriaBuilder builder,
        ...
        if (requestForm.isReqularized())
            predicate.getExpressions()
                    .add(builder.equal(siteDetailJoin.get("isappForRegularization"), requestForm.isReqularized()));
