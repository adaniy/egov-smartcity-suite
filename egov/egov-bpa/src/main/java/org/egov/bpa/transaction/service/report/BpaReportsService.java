...
public class BpaReportsService {
    ...
    private Set<BpaRegisterReportHelper> buildBpaRegisterReportResponse(final Page<BpaApplication> bpaApplications,
        ...
        for (BpaApplication application : bpaApplications) {
            ...
            List<OccupancyCertificate> ocs = occupancyCertificateService
                    .findByPermitNumber(application.getPlanPermissionNumber());
            if (!ocs.isEmpty()) {
                OccupancyCertificate oc = ocs.get(0);
                bpaRegister.setOcNumber(oc.getOccupancyCertificateNumber());
                ;
                bpaRegister.setOcApprovalDate(oc.getApprovalDate());
            }
            ...
            if (application.getStatus().getCode().equalsIgnoreCase(APPLICATION_STATUS_CANCELLED)
                    || application.getStatus().getCode().equalsIgnoreCase(APPLICATION_STATUS_REJECTED)) {
