...
import org.egov.infra.elasticsearch.service.EsApplicationIndexService;
...
public class BpaIndexService {
    ...
    @Autowired
    private EsApplicationIndexService applicationIndexService;
    ...
    public BpaIndex createBpaIndex(final BpaApplication bpaApplication) {
        ...
        if (!bpaApplication.getRevocations().isEmpty())
            for (PermitRevocation revoke : bpaApplication.getRevocations())
        ...
        if (bpaApplication.getPermitCancel() != null)
            bpaIndex.setPermitCancelDate(bpaApplication.getPermitCancel().getCancellationDate());
    ...
    private void buildBpaSiteDetails(BpaApplication bpaApplication, BpaIndex bpaIndex) {
        if (!bpaApplication.getSiteDetail().isEmpty()) {
            ...
            bpaIndex.setNoOfTowers(bpaApplication.getSiteDetail().get(0).getErectionoftower() == null
                    ? BigDecimal.ZERO
                    : bpaApplication.getSiteDetail().get(0).getErectionoftower());
            bpaIndex.setNoOfPoles(bpaApplication.getSiteDetail().get(0).getNoOfPoles() == null
                    ? BigDecimal.ZERO
                    : bpaApplication.getSiteDetail().get(0).getNoOfPoles());
            bpaIndex.setNoOfHutOrSheds(bpaApplication.getSiteDetail().get(0).getNoOfHutOrSheds() == null
                    ? BigDecimal.ZERO
                    : bpaApplication.getSiteDetail().get(0).getNoOfHutOrSheds());
            bpaIndex.setRoofConversion(bpaApplication.getSiteDetail().get(0).getRoofConversion() == null
                    ? BigDecimal.ZERO
                    : bpaApplication.getSiteDetail().get(0).getRoofConversion());
            bpaIndex.setNoOfShuttersOrDoors(bpaApplication.getSiteDetail().get(0).getShutter() == null
                    ? BigDecimal.ZERO
                    : bpaApplication.getSiteDetail().get(0).getShutter());
            bpaIndex.setNoOfWells(bpaApplication.getSiteDetail().get(0).getDwellingunitnt() == null
                    ? BigDecimal.ZERO
                    : bpaApplication.getSiteDetail().get(0).getDwellingunitnt());
            bpaIndex.setLengthOfCompoundWall(bpaApplication.getSiteDetail().get(0).getLengthOfCompoundWall() == null
                    ? BigDecimal.ZERO
    ...
    public void updateIndexes(final BpaApplication bpaApplication) {
        ...
        if (applicationIndex != null && bpaApplication.getId() != null)
            ...
            List<AppConfigValues> appConfigValue = appConfigValuesService.getConfigValuesByModuleAndKey(
                    BpaConstants.APPLICATION_MODULE_TYPE,
                    APP_CONFIG_KEY);
            ...
            applicationIndex = ApplicationIndex.builder().withModuleName(BpaConstants.APPL_INDEX_MODULE_NAME)
                    ...
                    .withAadharNumber(StringUtils.isBlank(bpaApplication.getOwner().getAadhaarNumber())
                            ? EMPTY
    ...
    private void buildApplicationIndexForUpdate(final BpaApplication bpaApplication, User user,
        ...
        if (bpaApplication.getStatus().getCode().equals(BpaConstants.APPLICATION_STATUS_CANCELLED)
                || bpaApplication.getStatus().getCode().equals(BpaConstants.APPLICATION_STATUS_REJECTED)) {
    ...
    private User getCurrentUser(final BpaApplication bpaApplication) {
        ...
        if (bpaApplication.getState() == null || bpaApplication.getState().getOwnerPosition() == null)
            ...
            Assignment assignment = bpaWorkFlowService.getApproverAssignmentByDate(bpaApplication.getState().getOwnerPosition(),
                    new Date());
            ...
            if (assignment != null) {
                ...
                assignments = bpaWorkFlowService.getAssignmentsByPositionAndDate(bpaApplication.getState()
